...
-- Função para simular Mouse.Hit no NPC
local function simulateMouseHit(targetPosition)
    if tool and tool:FindFirstChild("Handle") then
        local origin = tool.Handle.Position
        -- Cria um CFrame mirando para o target
        local hitCFrame = CFrame.new(origin, targetPosition)
        tool.Handle.CFrame = hitCFrame
    end
end
...
local function chase()
    ...
    while chasing do
        ...
        -- Rotacionar NPC continuamente para o jogador (horizontal)
        ...
        if dist <= attackRange and tick() - lastAttackTime >= attackCooldown then
            -- Simula Mouse.Hit apontando para o player
            simulateMouseHit(targetHRP.Position)
            -- Ativa a ferramenta
            tool:Activate()
            lastAttackTime = tick()
        end
